...
function TapCard({ onTap, onLongPress, style, children }) {
  ...
  const startPress = (e) => {
    ...
    timer.current = setTimeout(() => { fired.current = true; if (onLongPress) onLongPress(); }, 500);
  };
  const movePress = (e) => {
    if (!timer.current) return;
    const pt = e.touches ? e.touches[0] : e;
    if (Math.abs(pt.clientX-startPos.current.x)>10 || Math.abs(pt.clientY-startPos.current.y)>10) {
      clearTimeout(timer.current); timer.current = null;
    }
  };
  const endPress = () => {
    if (timer.current) { clearTimeout(timer.current); timer.current = null; }
    if (!fired.current && onTap) onTap();
    fired.current = false;
  };
  const cancelPress = () => { if (timer.current) clearTimeout(timer.current); timer.current = null; fired.current = false; };
  return (
    <div style={{...style, userSelect:"none", WebkitUserSelect:"none"}}
      onTouchStart={startPress} onTouchMove={movePress} onTouchEnd={endPress} onTouchCancel={cancelPress}
      onMouseDown={startPress} onMouseMove={movePress} onMouseUp={endPress} onMouseLeave={cancelPress}>
      {children}
    </div>
  );
}
...
function SearchScreen({ allOwned, onTeamClick, onBack, T }) {
  ...
  const results = useMemo(() => {
    ...
    Object.values(COLLECTIONS).forEach(coll => {
      const hits = coll.cards.filter(match).slice(0, 20);
      if (hits.length > 0) out[coll.id] = { name: coll.name, hits };
    });
    return out;
  }, [query]);
      ...
      <div style={{padding:"12px 16px"}}>
        {!query.trim() && (
          <div style={{textAlign:"center",padding:48,color:T.textDim}}>
            ...
            <div style={{fontSize:15,fontWeight:600}}>Busca en todas las colecciones</div>
    ...
    <div style={{
      ...
      <div style={{marginBottom:32,display:"flex",justifyContent:"center"}}>
        <div style={{
          ...
          <CromaLogo height={120} color="#ffffff"/>
        </div>
      </div>
  ...
  const loadFromCloud = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        data.forEach(row => {
          if (!cloudOwned[row.coll_id]) cloudOwned[row.coll_id] = {};
          if (!cloudRepeats[row.coll_id]) cloudRepeats[row.coll_id] = {};
          cloudOwned[row.coll_id][row.card_id] = row.owned;
          if (row.repeats > 0) cloudRepeats[row.coll_id][row.card_id] = row.repeats;
        });
        console.log(`✅ ${data.length} cartas en la nube`);
      } else {
        console.log('☁️ Nube vacía');
      }
      ...
      Object.keys(COLLECTIONS).forEach(collId => {
        ...
        const localOwned = LS.loadOwned(collId);
        const localRepeats = LS.loadRepeats(collId);
        
        const localOwnedCount = Object.keys(localOwned).filter(k => localOwned[k]).length;
        const cloudOwnedCount = Object.keys(cloudOwned[collId] || {}).filter(k => cloudOwned[collId][k]).length;
        
        console.log(`🔄 ${collId}: Local=${localOwnedCount} cartas, Nube=${cloudOwnedCount} cartas`);
        
        // Inicializar con nube
        mergedOwned[collId] = {...(cloudOwned[collId]||{})};
        mergedRepeats[collId] = {...(cloudRepeats[collId]||{})};
        
        // MERGE: Agregar cartas que existen solo en local
        ...
        // MERGE: Tomar el máximo de repeats entre local y nube
        Object.keys(localRepeats).forEach(cardId => {
          const localRep = localRepeats[cardId] || 0;
